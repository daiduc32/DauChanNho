    ...
    <div class="col-12 flex-grow-1">
        <div class="row h-100">
                ...
                <div class="col-auto position-relative" id="search-area">
                    <div class="form-group">
                        ...
                        <div class="input-group">
                            <input type="search" id="search" class="form-control rounded-0" autocomplete="off" placeholder="Tìm kiếm người dùng">
                            <button type="button" class="btn bg-transparent" style="margin-left: -35px; z-index: 100;" onclick="$('#search').val('').trigger('focus')">
                                <i class="fa fa-times"></i>
                            </button>
                        </div>

                        <div class="w-100 position-absolute bg-light d-none rounded-0 border border-dark" id="search-suggest">
                ...
                <div class="card shadow-sm rounded-0 flex-grow-1">
                    ...
                    <div class="card-body  rounded-0 overflow-auto">
                        <div class="list-group" id="convo_list">
                            <?php
                            if (isset($_GET['uid'])) {
                                $conn->query("UPDATE `messages` set status = 1 where to_user = '{$_SESSION['id']}' and md5(`from_user`) = '{$_GET['uid']}' ");
                            }
                            $convo_qry = $conn->query("SELECT *,CONCAT_WS(' ',firstname, middlename, lastname) as name FROM clients where id in (SELECT to_user FROM messages where from_user = '{$_SESSION['id']}') or id in (SELECT from_user FROM messages where to_user = '{$_SESSION['id']}')");
                            ...
                            <?php foreach ($convo_list_arr as $row) : ?>
                                <a href="./?uid=<?php echo md5($row['id']) ?>" class="list-group-item list-group-item-action list-item rounded-0 convo_with" data-id='<?php echo $row['id'] ?>'>
                                    <div class="w-100 d-flex position-relative">
                                        <div class="">
                                            <span class="rounded-circle notif-count badge bg-danger"><?php echo $row['un_read'] ?></span>
                                        </div>
                                        <div class="col-10 lh-1">
                                            <div class='text-truncate' title="<?php echo $row['name'] . ' ' . $row['email'] ?>"><b class="user-name"><?php echo $row['name'] ?> </b></div>
                                            <div class="text-muted search-user-email text-truncate last-message-field" title="<?php echo $row['last_message'] ?>"><?php echo ($row['last_message_id'] == $_SESSION['id'] ? 'Bạn : ' : '') . $row['last_message'] ?></div>
                                        </div>
                                    </div>
                                </a>
                            <?php endforeach; ?>
            ...
            <div class="col-md-8 h-100" id="right-panel">
                ...
                $qry = $conn->query("SELECT *,CONCAT(firstname,' ',middlename,' ',lastname) as fullname FROM  `clients` where md5(id) = '{$uid}' ");
                if ($qry->num_rows > 0) :
                            ...
                            <div class="col-auto flex-grow-1 px-1 py-1" id="">
                                <div id="convo-field">
                                    ...
                                    <div class="d-none" id="end-prev-data">
                                        <center><small class='text-muted'>Cuộc trò chuyện đã được bắt đầu tại đây</small></center>
                                    </div>
                                </div>
                            </div>
                                ...
                                <form action="" id="message-form">
                                    ...
                                    <div class="row align-items-center">
                                        ...
                                        <div class="col-auto flex-grow-1 mx-0">
                                            <textarea name="message" autofocus id="message" rows="2" class="form-control"></textarea>
                                        </div>
                                        <div class="col-auto">
                ...
                <?php else : ?>
                    <div class="card h-100 shadow-lg rounded-0">
                        <div class="d-flex flex-column h-100 w-100 align-items-center justify-content-center">
                            ...
                            <div class="text-muted">Bắt đầu cuộc trò chuyện và gửi lời chào tới ai đó nhé !</div>
                        </div>
                    </div>
                <?php endif; ?>
            </div>
...
<div id="bubble_clone" class="d-none">
    <div class="w-100 bubble-from d-flex align-items-start my-2">
        ...
        <div class="col-2 px-2"><a href="javascript:void(0)" class="text-muted delete-message">
...
<div id="convo-user-clone" class=" d-none">
    <a href="#" class="list-group-item list-group-item-action list-item rounded-0 convo_with" data-id=''>
        <div class="w-100 d-flex">
            ...
            <div class="flex-grow-1 lh-1">
                <div class='text-truncate'><b><span class="user-name"></span></b></div>
...
<script>
                    ...
                    $.ajax({
                        ...
                        success: function(resp) {
                            if (resp.length > 0) {
                                ...
                                process.then(() => {
                                    $('#convo-field').animate({
                                        scrollTop: $('#convo-field').get(0).scrollTop - 150
                                    }, 'fast')
                                    $('#prev-loader').addClass('d-none')
                                })
    ...
    function delete_message($id = '') {
        var _conf = confirm('Bạn chắc chắn muốn thu hồi tin nhắn này chứ ?')
        if (_conf == true) {
            $.ajax({
                ...
                success: function(resp) {
                    if (resp.status == 'success') {
                        $('.bubble-from[data-id="' + $id + '"] .bubble').addClass('deleted').text('Tin nhắn đã bị thu hồi')
    ...
    $(function() {
        ...
        Object.keys(messages).map(k => {
            ...
            if (messages[k].delete_flag == 1) {
                bubble.find('.bubble').addClass('deleted').text('Tin nhắn đã bị thu hồi')
        ...
        deleteInterval = setInterval(() => {
            ...
            $.ajax({
                ...
                success: function(resp) {
                    if (resp.length > 0) {
                        Object.keys(resp).map(k => {
                            $('.bubble-to[data-id="' + resp[k] + '"] .bubble').addClass('deleted').text('Tin nhắn đã bị thu hồi')
                ...
                success: function(resp) {
                    if (resp.length > 0) {
                        ...
                        Object.keys(resp).map(k => {
                            ...
                            last_id = resp[k].id
                            resp[k].message = (resp[k].from_user == sid) ? "Bạn : " + resp[k].message : resp[k].message;
                            if ($('.convo_with[data-id="' + convo_with + '"]').length > 0) {
                                ...
                                convo.attr('href', "./?uid=<?php echo isset($_GET['uid']) ? $_GET['uid'] : '' ?>")
                    ...
                    if (resp.length > 0) {
                        Object.keys(resp).map(k => {
                            ...
                            if ($('.convo_with[data-id="' + convo_with + '"]').length > 0) {
                                ...
                                var convo = $('#convo-user-clone .convo_with').clone()
                                convo.attr('href', "./?uid=" + resp[k].uid)
