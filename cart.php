    ...
    <div class="container">
        <div class="row">
            <div class="col d-flex justify-content-end mb-2">
                <button class="btn btn-outline-dark btn-flat btn-sm" type="button" id="empty_cart">Xóa toàn bộ sản phẩm trong giỏ hàng</button>
            </div>
        </div>
        <div class="card rounded-0">
            <div class="card-body">
                ...
                ?>
                    <div class="d-flex w-100 justify-content-between  mb-2 py-2 border-bottom cart-item">
                        <div class="d-flex align-items-center col-8">
                            <span class="mr-5"><a href="javascript:void(0)" class="btn btn-sm btn-outline-danger rem_item" data-id="<?php echo $row['id'] ?>"><i class="fa fa-trash"></i></a></span>
                            ...
                            <div class="ml-5">
                                ...
                                <div class="input-group">
                                    ...
                                    <input type="number" style="max-width: 15%;" class="form-control form-control-sm qty text-center cart-qty" placeholder="" aria-label="Example text with button addon" value="<?php echo $row['quantity'] ?>" aria-describedby="button-addon1" data-id="<?php echo $row['id'] ?>" readonly>
                ...
                <div class="d-flex w-100 justify-content-between mb-2 py-2 border-bottom">
                    <div class="col-10 d-flex justify-content-end"><h4>Thành tiền :</h4></div>
                    <div class="col d-flex justify-content-end"><h4 id="grand-total">-</h4></div>
                </div>
            </div>
        </div>
        <div class="d-flex w-100 justify-content-end">
            <a href="http://localhost:81/p1/shop.php?p=checkout" class="btn btn-sm btn-flat btn-dark">Thanh toán</a>
        </div>
    </div>
</section>
<script>
function calc_total() {
    ...
    $('.total-amount').each(function () {
        ...
        amount = amount.replace(/\,/g, ''); // Loại bỏ dấu phẩy
        amount = parseFloat(amount);
        total += amount;
    });

    $('#grand-total').text(total.toLocaleString('en-US')); 
}
...
function qty_change($type, _this) {
    ...
    price = parseFloat(price.replace(/\,/g, '')); // Loại bỏ dấu phẩy trước khi tính toán

    new_total = parseFloat(qty * price);
    _this.closest('.cart-item').find('.cart-qty').val(qty);

    _this.closest('.cart-item').find('.total-amount').text(new_total.toLocaleString('en-US'));
    ...
    calc_total();
        ...
        $.ajax({
            ...
            error:err=>{
                console.log(err)
                alert_toast(" Đã xảy ra lỗi, vui lòng thao tác lại", 'error');
                end_loader()
            },
    ...
    $(function(){
        ...
        $('#empty_cart').click(function(){
            // empty_cart()
            _conf("Bạn chắc chắn muốn làm rỗng giỏ hàng chứ ?",'empty_cart',[])
        })
        $('.rem_item').click(function(){
            _conf("Bạn chắc chắn muốn xóa sản phẩm này khỏi giỏ hàng chứ ?",'rem_item',[$(this).attr('data-id')])
        })
    })
</script>