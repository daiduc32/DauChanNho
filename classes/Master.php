...
Class Master extends DBConnection {
	...
	function delete_adopt(){
		extract($_POST);
		$delete = $this->conn->query("DELETE FROM `adopt_pet` where pet_id = '{$id}'");
		if($this->capture_err())
			return $this->capture_err();
		if($delete){
			$resp['status'] = 'success';
	...
	function delete_user(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Tài khoản đã bị xóa thành công !");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function delete_blog(){
		...
		if($del){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Bài viếtviết đã bị xóa thành công !");
		}else{
			$resp['status'] = 'failed';
			$resp['error'] = $this->conn->error;
		}
	...
	function manage_user(){
		...
		if($check > 0){
			$resp['status'] = 'failed';
			$resp['msg'] = "Email này đã được sử dụng !";
			return json_encode($resp);
			exit;
		}
	...
	function manage_blog(){
		...
		if($save){
			$resp['status'] = 'success';
				$this->settings->set_flashdata('success',"Sửa bài bài viết thành công !");
	...
	function manage_adopt(){
		...
		if(!empty($data)){
			$sql = "UPDATE `adopt_pet` SET {$data} where pet_id = '{$pet_id}' ";
			$save = $this->conn->query($sql);
		}else{
			exit;
		}
		if($save){
			$resp['status'] = 'success';
			$this->settings->set_flashdata('success',"Lưu thông tin thành công !");
...
case 'manage_user':
	...
	break;
	case 'manage_adopt':
		echo $Master->manage_adopt();
	break;
	case 'delete_adopt':
		echo $Master->delete_adopt();
	break;
	case 'delete_blog':
